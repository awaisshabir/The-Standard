...
We then call the logical validation methods `IsInvalid`, `IsValuesNotSame`, `IsDatesNotSame` and `IsNotRecent` to ensure our conditional requirements are met. Here are the example implementations for these methods:
