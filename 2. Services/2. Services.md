...
ValueTask<StudentV2> RetrieveV2StudentV2ByIdAsync(Guid studentId)
```
...
- A versioned service named after the model version `{Entity}V{n}Service.cs`
  - Any behaviour versioning in the method name, not the service name, to avoid confusion with model versioning.
    - V0 -> `ValueTask<EventAddressV1> AddEventAddressV1Async(EventAddressV1 eventAddress);`
...
- A versioned service with both model and behavior changes `{Entity}V{n}Service.cs`
...
```csharp
// Public localization exception - meaningful to consumers
...
// Internal categorization exception - used only within the library
...
// Public contract for the root client
public interface IUniversityClient
{
    IStudentClient Students { get; }
}

// Public exposer (entry point)
