...
- When the model version changes, the service filename increments to reflect the new model version, and the behavior version resets to V0:
  - `{Entity}V{n}Service.cs`
  - The method naming resets to the implied V0 convention:
    - V0 -> `ValueTask<EventAddressV2> AddEventAddressV2Async(EventAddressV2 eventAddress);`
