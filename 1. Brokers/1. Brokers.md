...
#### 1.9.0.0 Special Considerations

The following concerns must be addressed before a new broker version is introduced.

- **Data Migration**: When data must be moved from one version to another, a migration strategy must be defined before the new version is introduced. Migration scripts or data transformation tooling must carry all records from the old version to the new while preserving every relationship and constraint.

- **Data Retention and Archiving**: When older versions of data must be retained for historical or auditing purposes, those records must be treated as read-only. An archiving strategy must be established so that older records remain retrievable without risk of accidental modification.

- **Deprecation**: When a version is ready to be retired, all consumers must be notified before removal begins. The older version must remain accessible until a defined end-of-life date, giving consumers sufficient time to migrate to the current version.

- **Schema Naming Conflicts**: When multiple versions of a model coexist in the same database, all table names, index names, and constraint names must include the version suffix to prevent naming collisions. This applies equally to Entity Framework fluent configurations and any raw migration scripts.


### 1.9.1 Pluralization

The plural `s` is always appended
So the pattern is:
...
### 1.9.2 File Name Conventions

File names follow a structured pattern
