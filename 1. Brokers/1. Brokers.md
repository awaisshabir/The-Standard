...
When brokers rely on external libraries or routines, The Standard mandates the use of Standardized Provider Abstraction Libraries. These libraries decouple brokers from any specific external provider, ensuring extensibility, distributability, and local mockability (Cloud-Foreign).

For full details, see [1.8 Broker Provider Abstraction](1.8%20Broker%20Provider%20Abstraction.md).

## 1.9 Versioning

## 1.9.0 Versioning Strategy

When a model is upgraded, a new version is introduced by appending a version suffix directly to the model
name — `EventAddress` becomes `EventAddressV1`, then `EventAddressV2`. Each version lives in its own
dedicated partial class file (e.g., `StorageBroker.EventAddressV1s.cs`, `StorageBroker.EventAddressV2s.cs`),
keeping concerns cleanly separated and leaving the original unversioned file untouched for backward
compatibility. 

Each version of the model has its own set of operations, following the same naming conventions but with the version suffix included.  This means each version is also persisted in its own table.

This allows older consumers of `EventAddress` or `EventAddressV1` to not be broken when a new
version is introduced. Newer versions can also introduce additional operations — for example, both `V1` and
`V2` expose a `DeleteAsync` method that does not exist on the original `EventAddresses` partial, reflecting
evolving capabilities without retroactively modifying the legacy surface.

** Special considerations 
 - Should data be migrated from one version to another. In that case, a migration strategy must be implemented to 
   ensure data integrity and consistency across versions. This may involve writing custom migration scripts or using a 
   data transformation tool to move data from the old version to the new version while maintaining relationships and constraints.

 - Should older versions of data be retained (read only version) or archived. Depending on the nature of the data and 
   the requirements of the system, it may be necessary to retain older versions of data for historical or auditing purposes. 
   In that case, an archiving strategy must be implemented to store older versions of data in a separate location or 
   format that allows for easy retrieval and analysis when needed. 

 - Should older versions be deprecated. In that case, a deprecation strategy must be implemented to inform users of 
   the older version about the upcoming changes and encourage them to migrate to the newer version. This may involve 
   providing documentation, support, and incentives for users to transition to the new version while still maintaining 
   support for the older version until a specified end-of-life date. 


## 1.9.1 Pluralization

The plural `s` is always appended **after** the full versioned type name, including the version suffix.
So the pattern is:

```
[Operation] + [ModelName] + [Version] + s + Async
```

| Model            | Method                          |
|------------------|---------------------------------|
| `EventAddress`   | `SelectAllEventAddressesAsync`  |
| `EventAddressV1` | `SelectAllEventAddressV1sAsync` |
| `EventAddressV2` | `SelectAllEventAddressV2sAsync` |

This means the plural `s` binds to the version tag, producing `V2s` rather than the potentially more
"natural" but non-standard `sV2`. The same convention applies consistently to the `DbSet` property names
(`EventAddressV2s`) and the file names (`StorageBroker.EventAddressV2s.cs`), keeping naming uniform across
the entire broker layer.

## File Name Conventions

File names follow a structured pattern built from up to four segments separated by dots:

```
[I]StorageBroker . [ModelName][Vn][s] . [Configurations] . cs
```

- The optional `I` prefix indicates an interface file.
- `[ModelName]` is the entity name as-is.
- `[Vn]` is the version suffix (`V1`, `V2`, etc.), omitted for the original unversioned model.
- `[s]` is appended only when the model name is **singular** — if the name already ends in `s` (e.g., `Archives`), no extra `s` is added.
- `.Configurations` is an optional segment used for Entity Framework fluent configuration partial files.

### Examples

| File | Breakdown |
|------|-----------|
| `IStorageBroker.EventAddressV2s.cs` | Interface · singular model `EventAddress` · version `V2` · plural `s` |
| `StorageBroker.EventAddressV2s.cs` | Implementation · singular model `EventAddress` · version `V2` · plural `s` |
| `StorageBroker.EventArchivesV2.Configurations.cs` | Implementation · already-plural model `EventArchives` · version `V2` · no extra `s` · EF config |
